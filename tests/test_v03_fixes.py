...
def test_pyproject_has_scikit_build_section():
    ...
    assert "scikit-build" in content.get(
        "tool", {}
...
def test_cibuildwheel_test_requires_has_pyyaml():
    ...
    assert any(
        "pyyaml" in r.lower() for r in test_requires
...
def test_cli_download_has_timeout():
    ...
    content = (ROOT / "meshmonk/cli.py").read_text()
    assert "timeout=30" in content, "_demo_download missing timeout=30 in urlopen call"


def test_cli_download_catches_urlerror():
    """cli.py _demo_download must catch URLError and OSError."""
    content = (ROOT / "meshmonk/cli.py").read_text()
    assert "URLError" in content, "_demo_download missing URLError in except clause"
...
def test_cli_download_cleanup_partial_file():
    ...
    content = (ROOT / "meshmonk/cli.py").read_text()
    assert (
        "unlink(missing_ok=True)" in content
...
def test_cli_download_raises_typer_exit():
    ...
    assert (
        content.count("raise typer.Exit(code=1)") >= 2
...
def test_types_md_rigid_transform_has_matrix_attr():
    ...
    assert (
        "`.matrix`" in content or "| `matrix`" in content
...
def test_types_md_rigid_transform_no_fake_attrs():
    ...
    assert (
        "`.rotation`" not in rt_section and "| `rotation`" not in rt_section
    ), "types.md RigidTransform still lists fake .rotation attribute"
    assert (
        "`.translation`" not in rt_section and "| `translation`" not in rt_section
    ), "types.md RigidTransform still lists fake .translation attribute"
    assert (
        "`.scale`" not in rt_section and "| `scale`" not in rt_section
...
def test_release_yml_workflow_level_permissions_empty():
    ...
    assert (
        workflow_perms == {} or workflow_perms is None
    ...
    if workflow_perms:
        assert (
            "id-token" not in workflow_perms
        ), "id-token: write must NOT be at workflow level"
        assert (
            "contents" not in workflow_perms
...
def test_release_yml_create_release_needs_build_jobs():
    ...
    if isinstance(needs, str):
        needs = [needs]
    assert "build-wheels" in needs, "create-release must need build-wheels"
    assert "build-sdist" in needs, "create-release must need build-sdist"
    assert (
        "publish-pypi" not in needs
