...
// All internal classes live in namespace registration (load-bearing code, do
// NOT modify). This file bridges the new value-based public API to the old
// pointer-based internal API.

#include "meshmonk/meshmonk.hpp"

// Internal class headers (in library/src/ — included via
...
static_assert(
    meshmonk::NUM_FEATURES == registration::NUM_FEATURES,
...
VecDynFloat run_final_inlier_pass(FeatureMat &aligned_features,
  ...
  FeatureMat corresponding_features =
      FeatureMat::Zero(N, registration::NUM_FEATURES);
  ...
  if (corr_params.symmetric) {
    ...
    f->set_parameters((size_t)corr_params.num_neighbours,
  ...
  inlier_det.set_input(&aligned_features, &corresponding_features,
                       &corresponding_flags);
  inlier_det.set_output(
      &final_weights); // output written in-place to final_weights
...
rigid_registration(Eigen::Ref<const FeatureMat> floating_features,
  ...
  if (target_flags.maxCoeff() == 0.0f)
    return tl::unexpected{RegistrationError::DegenerateInput};
  // Zero-range bounding box check — degenerate only if ALL three axes have zero
  // range
  if (floating_features.col(0).maxCoeff() ==
          floating_features.col(0).minCoeff() &&
      floating_features.col(1).maxCoeff() ==
          floating_features.col(1).minCoeff() &&
      floating_features.col(2).maxCoeff() ==
          floating_features.col(2).minCoeff())
    return tl::unexpected{RegistrationError::DegenerateInput};
  ...
  registrator.set_input(&floating_copy, &target_copy, &floating_flags_copy,
                        &target_flags_copy);
  registrator.set_parameters(params.correspondences.symmetric,
                             ...
                             (size_t)params.num_iterations, params.use_scaling);
  registrator.update();
...
nonrigid_registration(Eigen::Ref<const FeatureMat> floating_features,
                      Eigen::Ref<const FeatureMat> target_features,
                      Eigen::Ref<const FacesMat> floating_faces,
                      Eigen::Ref<const FacesMat> target_faces,
                      Eigen::Ref<const VecDynFloat> floating_flags,
  ...
  registrator.set_parameters(
      ...
      (size_t)params.correspondences.num_neighbours,
      params.correspondences.flag_threshold,
      params.correspondences.equalize_push_pull, params.inliers.kappa,
      params.inliers.use_orientation, (size_t)params.num_iterations,
...
pyramid_registration(Eigen::Ref<const FeatureMat> floating_features,
                     Eigen::Ref<const FeatureMat> target_features,
                     Eigen::Ref<const FacesMat> floating_faces,
                     Eigen::Ref<const FacesMat> target_faces,
                     Eigen::Ref<const VecDynFloat> floating_flags,
  ...
  registrator.set_input(floating_copy, target_copy, floating_faces_copy,
                        target_faces_copy, floating_flags_copy,
                        target_flags_copy);
  registrator.set_parameters(
      (size_t)params.num_iterations, (size_t)params.num_pyramid_layers,
      params.downsample.float_start, params.downsample.target_start,
      params.downsample.float_end, params.downsample.target_end,
      params.correspondences.symmetric,
      (size_t)params.correspondences.num_neighbours,
      params.correspondences.flag_threshold,
      params.correspondences.equalize_push_pull, params.inliers.kappa,
      params.inliers.use_orientation, params.transform.sigma,
      (size_t)params.transform.num_viscous_iterations_start,
      (size_t)params.transform.num_viscous_iterations_end,
      (size_t)params.transform.num_elastic_iterations_start,
      (size_t)params.transform.num_elastic_iterations_end);
  ...
  result.displacement_field = floating_copy.leftCols(3) - original_positions;
  result.per_layer_iterations =
      std::vector<int>(params.num_pyramid_layers, iters_per_layer);
  return result;
}
...
compute_correspondences(Eigen::Ref<const FeatureMat> floating_features,
                        Eigen::Ref<const FeatureMat> target_features,
                        Eigen::Ref<const VecDynFloat> floating_flags,
                        Eigen::Ref<const VecDynFloat> target_flags,
                        bool symmetric, int num_neighbours,
                        float flag_threshold, bool equalize_push_pull) {
...
VecDynFloat
compute_inlier_weights(Eigen::Ref<const FeatureMat> floating_features,
                       Eigen::Ref<const FeatureMat> corresponding_features,
                       Eigen::Ref<const VecDynFloat> corresponding_flags,
                       float kappa, bool use_orientation) {
  FeatureMat floating_copy = floating_features;
...
RigidTransform
compute_rigid_transform(Eigen::Ref<const FeatureMat> floating_features,
                        Eigen::Ref<const FeatureMat> corresponding_features,
                        Eigen::Ref<const VecDynFloat> weights,
                        bool use_scaling) {
  // All 4 steps required — RigidTransformer uses set_output target as
  // BOTH input for centroid computation AND write target; omitting set_output =
  // null crash.
  FeatureMat floating_copy = floating_features;
  ...
  t.set_input(&corresponding_copy,
              &weights_copy);   // (1) correspondences + weights
  t.set_output(&floating_copy); // (2) REQUIRED: used as both centroid input and
                                // write target
...
FeatureMat compute_nonrigid_transform(
    Eigen::Ref<const FeatureMat> floating_features,
    Eigen::Ref<const FeatureMat> corresponding_features,
    Eigen::Ref<const FacesMat> floating_faces,
    Eigen::Ref<const VecDynFloat> floating_flags,
    Eigen::Ref<const VecDynFloat> weights, int num_smoothing_neighbours,
    float sigma, int num_viscous_iterations, int num_elastic_iterations) {
  FeatureMat floating_copy = floating_features;
  FeatureMat corr_features_copy = corresponding_features;
  ...
  transformer.set_input(&corr_features_copy, &weights_copy, &flags_copy,
                        &faces_copy);
  transformer.set_output(&floating_copy);
  transformer.set_parameters((size_t)num_smoothing_neighbours, sigma,
...
downsample_mesh(Eigen::Ref<const FeatureMat> features,
                Eigen::Ref<const FacesMat> faces,
                Eigen::Ref<const VecDynFloat> flags, float downsample_ratio) {
...
FeatureMat scale_shift_mesh(Eigen::Ref<const FeatureMat> previous_features,
...
Vec3Mat compute_normals(Eigen::Ref<const Vec3Mat> positions,
                        Eigen::Ref<const FacesMat> faces) {
  Vec3Mat positions_copy = positions;
  FacesMat faces_copy = faces;
  Vec3Mat normals(positions_copy.rows(), 3);

  registration::update_normals_for_altered_positions(positions_copy, faces_copy,
                                                     normals);
