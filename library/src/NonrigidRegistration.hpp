...
class NonrigidRegistration {
  /*
  # GOAL
  This class performs icp-based nonrigid registration between two oriented
  pointclouds.
...
public:
  ...
  void set_parameters(bool symmetric, size_t numNeighbours, float flagThreshold,
                      ...
                      float sigmaSmoothing, size_t numViscousIterationsStart,
...
private:
  ...
  // # Internal Parameters
  // ## Transformation
  float _viscousAnnealingRate = exp(
      log(float(_numViscousIterationsEnd) / float(_numViscousIterationsStart)) /
      _numIterations);
  float _elasticAnnealingRate = exp(
      log(float(_numElasticIterationsEnd) / float(_numElasticIterationsStart)) /
      _numIterations);
